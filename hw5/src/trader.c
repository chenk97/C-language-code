...
int trader_init(void){
    ...
    if((trader = (TRADER *)malloc(sizeof(TRADER))) == NULL) return -1;
    trader -> links.PREV = trader -> links.NEXT = NULL;
    return 0;
}
...
TRADER *trader_login(int fd, char *name){
    ...
    while(cur -> links.NEXT){
        ...
        if(!strcmp(cur -> name, name)){
            if(cur -> ref == 1){
                cur -> connfd = fd;
                cur = trader_ref(cur, "because trader has logged in");
                pthread_mutex_unlock(&mutex);
                return cur;
            /**********?????*********/
            }else if (cur -> ref == 2){
                int temp = cur -> connfd;/*save the original fd*/
                cur -> connfd = fd;
                debug("**********trader has already logged in**********");
                trader_send_nack(cur);
                cur -> connfd = temp;/*restore fd*/
                pthread_mutex_unlock(&mutex);
                return NULL;
            }
        }
    }
    TRADER * new = (TRADER *)Malloc(sizeof(TRADER));
    new -> links.PREV = new -> links.NEXT = NULL;
    new -> connfd = fd;
    new -> name = name;
    new -> ref = 0;
    new = trader_ref(new, "for newly created trader");
    new -> balance = 0;
    new -> inventory = 0;
    new = trader_ref(new, "because trader has logged in");
    cur -> links.NEXT = new;
    new -> links.PREV = cur;
    cnt ++;
    pthread_mutex_unlock(&mutex);
...
TRADER *trader_ref(TRADER *trader, char *why){
    trader -> ref ++;
    debug("(%d -> %d) %s", trader -> ref - 1, trader -> ref, why);
    return trader;
}
...
void trader_unref(TRADER *trader, char *why){
    trader -> ref --;
    if(!trader -> ref){/*if reference == 0*/
        TRADER *prev = trader -> links.PREV;
        TRADER *next = trader -> links.NEXT;
...
int trader_send_packet(TRADER *trader, BRS_PACKET_HEADER *pkt, void *data){
    if(proto_send_packet(trader -> connfd, pkt, data)) return -1;
    return 0;
}
...
int trader_broadcast_packet(BRS_PACKET_HEADER *pkt, void *data){
    ...
    while(cur -> links.NEXT){
        cur = cur -> links.NEXT;
        if(trader_send_packet(cur, pkt, data)) return -1;
    }
    return 0;
}
...
int trader_send_ack(TRADER *trader, BRS_STATUS_INFO *info){
    ...
    if(!info) hdr->size = 0;
        ...
        hdr -> size = htons(sizeof(*info));/*htons()*/
...
int trader_send_nack(TRADER *trader){
    ...
    struct timespec tsp;
    clock_gettime(CLOCK_MONOTONIC, &tsp);
    hdr -> timestamp_sec = htonl(tsp.tv_sec);
    hdr -> timestamp_nsec = htonl(tsp.tv_nsec);
    hdr -> size = 0;
...
int trader_decrease_balance(TRADER *trader, funds_t amount){
    funds_t balan = trader -> balance;
    if(amount > balan) return -1;
    trader -> balance -= amount;
    return 0;
}
...
int trader_decrease_inventory(TRADER *trader, quantity_t quantity){
    quantity_t invent = trader -> inventory;
    if(quantity > invent) return -1;
    trader -> inventory -= quantity;
    return 0;
}
