...
Test(sf_memsuite_student, malloc_free_edge_block, .init = sf_mem_init, .fini = sf_mem_fini) {
	...
	sf_block *bp = (sf_block *)((char*)x - 2*sizeof(sf_header));
	cr_assert(bp->header & THIS_BLOCK_ALLOCATED, "Allocated bit is not set!");
	cr_assert((bp->header & BLOCK_SIZE_MASK) == 4048, "Realloc'ed block size not what was expected!");
...
Test(sf_memsuite_student, malloc_two_and_free, .init = sf_mem_init, .fini = sf_mem_fini) {
	void *x = sf_malloc(4032);
	void *y = sf_malloc(8*sizeof(int));

	cr_assert_not_null(x, "x is NULL!");
	cr_assert_not_null(y, "y is NULL!");

	sf_block *bp1 = (sf_block *)((char*)x - 2*sizeof(sf_header));
	cr_assert(bp1->header & THIS_BLOCK_ALLOCATED, "Allocated bit is not set!");
	cr_assert((bp1->header & BLOCK_SIZE_MASK) == 4048, "Realloc'ed block size not what was expected!");

	sf_block *bp2 = (sf_block *)((char*)y - 2*sizeof(sf_header));
	cr_assert(bp2->header & THIS_BLOCK_ALLOCATED, "Allocated bit is not set!");
	cr_assert((bp2->header & BLOCK_SIZE_MASK) == 48, "Realloc'ed block size not what was expected!");

	sf_free(x);
	sf_free(y);

	// After realloc'ing x, we can return a block of size 8144 to the freelist.
	// This block will go into the main freelist and be coalesced.
	assert_free_block_count(0, 1);
	assert_free_block_count(8144, 1);
}

Test(sf_memsuite_student, realloc_smaller_block_edge_block, .init = sf_mem_init, .fini = sf_mem_fini) {
	void *x = sf_malloc(4032);
	void *y = sf_realloc(x, 5*sizeof(double));
	...
	// After realloc'ing x, we can return a block of size 3984 to the freelist.
	// This block will go into the main freelist and be coalesced.
	assert_free_block_count(0, 1);
...
Test(sf_memsuite_student, realloc_invalid_pointer, .init = sf_mem_init, .fini = sf_mem_fini) {
	void *y = sf_realloc(sf_mem_start()+10, 0);
...
Test(sf_memsuite_student, realloc_memcpy_check, .init = sf_mem_init, .fini = sf_mem_fini) {
	double* ptr = sf_malloc(sizeof(double));

    *ptr = 10;

    double *rptr = sf_realloc(ptr, 2*sizeof(double));

    double* ptr2 = sf_malloc(sizeof(double));

    *ptr2 = 10;

    double *rptr2 = sf_realloc(ptr2, sizeof(int));

    cr_assert(*ptr == *rptr, "memory copy failed");
    cr_assert(*ptr2 == *rptr2, "memory copy failed");
}
