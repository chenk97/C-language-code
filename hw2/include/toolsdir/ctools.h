...
void save_and_exit();
//rolo.c
void/*int*/ cathelpfile (char *filepath, char *helptopic, int clear);
...
//operation.c
int rolo_menu_yes_no (char *prompt,
  int rtn_default,
  int help_allowed,
  char *helpfile, char *subject);
...
//search.c
int rolo_menu_data_help_or_abort (char *prompt, char *helpfile,
  char *subject, char **ptr_response);

void display_field_names ();

int rolo_menu_number_help_or_abort (char *prompt,int low,int high,int *ptr_ival);

int get_person(int argc, char** argv);

typedef struct person_name{
        char* name;
        struct person_name *next;
} Name_list;
...
extern int nbuffconcat(char *buffer,
  int n,...);
...
extern int/*RETURN INT AS ANNOTATED...*/ read_yes_or_no ();
