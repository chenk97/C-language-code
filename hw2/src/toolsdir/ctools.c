...
void copy (dest,src,n)
  ...
  { register int j = 0;
    while (j++ < n) *dest++ = *src++;//copy src to dest
  }
...
{
  register char ch;
  while ((astring != NULL) && (ch = *astring++) != '\0') {
    if (ch == ' ' || ch == '\t') continue;
    return(F);
  }
  return(T);
...
int mygetline (iport,buffer,buflen) FILE *iport; char *buffer; int buflen;
...
extern int read_file_into_buffer (filename,ptr_lines,maxlines,buffer,buflen,linebuffer,linebuflen)
  char *filename;
  char ***ptr_lines;
  int maxlines;
  char *buffer;
