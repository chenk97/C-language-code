...

{
  ...
  for (j = 0; j < 100; j++) otherdata[j] = 0;/*otherdata[0-99]=0*/
  for (j = 0; j < N_BASIC_FIELDS; j++) basicdata[j] = 0;/*basicdata[0-7]=0*/
  ...
  basicdata[N_BASIC_FIELDS - 1] = timestring();/*basicdata[8-1]=currentTime*/
...
void/*default int*/ display_list_of_entries (rlist) Ptr_Rolo_List rlist;
...
void/*default int*/ rolo_peruse_mode (first_rlink) Ptr_Rolo_List first_rlink;
