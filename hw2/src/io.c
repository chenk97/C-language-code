...
//#include <sys/statvfs.h>//space check
...

{
  ...
  while (j < filesize) {
      ...
      if (rolofiledata[j] == '\n') {
         ...
         //write 0 to other_fields at address newentry
         continue;
      }

      /* there must be additional, user-inserted fields. Find out how many. */

      start_of_others = j;//j not initialized???
      while (1) {
        while (rolofiledata[j] != '\n') {
          j++;
        }
     ...
     //write other pointers to other_fields at address new entry
...

{
  ...
  for (j = 0; j < N_BASIC_FIELDS - 1; j++) {
      string = get_basic_rolo_field(j,entry);/*get the string in field*/
      //if(string != NULL){
...
void/*int*/ cathelpfile (filepath,helptopic,clear)
...
void/*default int*/ any_char_to_continue ()
{
  char buffer[80];
