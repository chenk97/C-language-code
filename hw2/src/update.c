...

{
  ...
  display_entry_for_update(updated ? entry : old_entry/*,stdout*/);
    ...
    case MENU_MATCH :
      ...
      switch (menuval) {
        ...
        case U_ABORT :
          ...
          //free all space allocated by copy_entry
          for(int i = 0; i < N_BASIC_FIELDS; i++){
            free(get_basic_rolo_field(i, entry));
          }
          for(int j = 0; j < get_n_others(entry); j++){
             free(get_other_field(j, entry));
          }
